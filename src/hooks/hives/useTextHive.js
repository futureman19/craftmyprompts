...
export const useTextHive = (initialKeys = {}) => {
    ...
    const startMission = async (prompt) => {
        ...
        try {
            ...
            setHistory([{ ...rawMsg, content: cleanContent, text: cleanContent, role: 'The Editor', type: 'vision_options' }]);
    ...
    const submitChoices = async (choices) => {
        ...
        try {
            ...
            const rawMsg = data.swarm[0];
            const cleanContent = cleanJson(rawMsg.content);
            setHistory(prev => [...prev, { ...rawMsg, content: cleanContent, text: cleanContent, role: 'The Linguist', type: 'spec_options' }]);
        } catch (e) { console.error(e); } finally { setLoading(false); }
    };
    ...
    const submitSpecs = async (specs) => {
        ...
        try {
            ...
            const rawMsg = data.swarm[0];
            const cleanContent = cleanJson(rawMsg.content);
            setHistory(prev => [...prev, { ...rawMsg, content: cleanContent, text: cleanContent, role: 'The Scribe', type: 'blueprint' }]); // Changed 'The Writer' to 'The Scribe' to match standard naming if needed, but sticking to 'The Scribe' as standard
        } catch (e) { console.error(e); } finally { setLoading(false); }
    };
    ...
    const writeManuscript = async () => {
        ...
        try {
            ...
            const rawMsg = data.swarm[0];
            const cleanContent = cleanJson(rawMsg.content);
            setHistory(prev => [...prev, { ...rawMsg, content: cleanContent, text: cleanContent, role: 'The Publisher', type: 'final' }]);
        } catch (e) { console.error(e); } finally { setLoading(false); }
    };
