...
const BuilderInputPanel = ({ 
    ...
    return (
        <div className="space-y-3">
            {/* --- TOPIC BOX (Compacted) --- */}
            <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-slate-200 dark:border-slate-700 relative group transition-all focus-within:ring-2 focus-within:ring-indigo-500/20">
                ...
                {detectedVars.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700 animate-in fade-in">
            ...
            {state.mode === 'text' && state.textSubMode === 'coding' && (
                <div className="bg-slate-900 rounded-xl p-3 border border-slate-700 shadow-sm animate-in slide-in-from-top-2 fade-in">
                    <div className="flex justify-between items-center mb-2">
                    ...
                    <textarea className="w-full p-2 bg-slate-950 text-slate-200 rounded-lg border border-slate-800 focus:ring-1 focus:ring-indigo-500 outline-none resize-none text-xs font-mono leading-relaxed placeholder-slate-600" rows={3} placeholder="// Paste your broken code or current file content here..." value={state.codeContext} onChange={(e) => dispatch({ type: 'UPDATE_FIELD', field: 'codeContext', value: e.target.value })} />
                </div>
            )}
            ...
            {state.mode === 'video' && (
                    ...
                    {/* Video Control Panel */}
                        <div className="bg-slate-100 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 space-y-3">
            ...
            {filteredData.map((category) => (
                <div key={category.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden h-fit">
                    <div className="p-3 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" onClick={() => toggleCategory(category.id)}>
                        <div className="flex items-center gap-2">
                            <div className={`p-1 rounded-md ${state.selections[category.id]?.length ? (state.mode === 'text' ? 'bg-indigo-100 text-indigo-600' : (state.mode === 'video' ? 'bg-purple-100 text-purple-600' : 'bg-pink-100 text-pink-600')) : 'bg-slate-100 dark:bg-slate-700 text-slate-400'}`}>{category.icon}</div>
                            <div><h2 className="font-semibold text-slate-700 dark:text-slate-200 text-sm">{category.title}</h2></div>
                        </div>
                        <ChevronDown size={14} className={`text-slate-400 transition-transform ${expandedCategories[category.id] ? 'rotate-180' : ''}`}/>
                    </div>
                    {expandedCategories[category.id] && (
                        <div className="px-3 pb-3 pt-1 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700">
                                ...
                                if (category.isVisual) {
                                    return (
                                        <div key={sub.name} className="mt-3">
                                            <h3 className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">{sub.name}</h3>
                                            <div className="grid grid-cols-2 gap-2">
                                                {optionsToShow.map((option) => {
                                                    ...
                                                    return (
                                                        <button key={option} onClick={() => toggleSelection(category.id, option)} className={`relative h-14 rounded-lg text-xs font-medium border overflow-hidden text-left p-2 flex flex-col justify-end transition-all ${isSelected ? (state.mode === 'video' ? 'border-purple-500 ring-2 ring-purple-200 bg-purple-50' : 'border-pink-500 ring-2 ring-pink-200 bg-pink-50') : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 hover:border-slate-300 dark:hover:border-slate-500 text-slate-700 dark:text-slate-300'}`}>
                                                            <div className={`absolute inset-0 ${opacityClass} transition-opacity`} style={{ background: `url('${previewUrl}') center/cover` }} /><span className="relative z-10 truncate w-full shadow-black drop-shadow-md text-shadow text-[10px]">{option}</span>
                                                        </button>
                                                    );
                                                })}
                                            </div>
                                            {hasMore && (<button onClick={() => toggleSubcatExpansion(sub.name)} className="mt-2 text-[10px] font-bold text-slate-500 hover:text-pink-600 flex items-center gap-1">{isExpanded ? 'Show Less' : `Show ${sub.options.length - visibleLimit} More...`}</button>)}
                                        </div>
                                    )
                                }
                                return (
                                            ...
                                            {optionsToShow.map((option) => {
                                                ...
                                                return (
                                                    <div key={option} className="flex items-center">
                                                        <button onClick={() => toggleSelection(category.id, option)} className={`px-2 py-0.5 rounded-md text-[11px] border transition-all ${isSelected ? (state.mode === 'text' ? 'bg-indigo-600 border-indigo-600 text-white' : (state.mode === 'video' ? 'bg-purple-600 border-purple-600 text-white' : 'bg-pink-600 border-pink-600 text-white')) : 'bg-white dark:bg-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:border-slate-300'}`}>{option}</button>
                                                        {isSelected && state.mode === 'art' && category.id !== 'params' && state.targetModel !== 'dalle' && state.targetModel !== 'gemini' && (
                                                            <div className="ml-1 flex items-center gap-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-md px-1 py-0.5 animate-in slide-in-from-left-2 fade-in duration-200">
                                                                <span className="text-[8px] text-slate-400 font-bold">W:</span>
                                                                <input type="range" min="0.1" max="2.0" step="0.1" value={selectionItem.weight} onChange={(e) => dispatch({ type: 'UPDATE_WEIGHT', payload: { categoryId: category.id, optionValue: option, newWeight: e.target.value } })} className="w-8 h-1 accent-pink-500 bg-slate-200 rounded-lg appearance-none cursor-pointer" />
                                                            </div>
                                                        )}
                                                    </div>
                                                );
                                            })}
                                            {hasMore && (<button onClick={() => toggleSubcatExpansion(sub.name)} className="px-2 py-0.5 rounded-md text-[10px] font-bold text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 transition-colors">{isExpanded ? 'Show Less' : `+${sub.options.length - visibleLimit}`}</button>)}
                                        </div>
                                    </div>
                                );
                            })}
