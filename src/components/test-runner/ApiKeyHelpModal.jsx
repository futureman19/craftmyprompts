...
const ApiKeyHelpModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row overflow-hidden h-[500px]">
                ...
                <div className="w-full md:w-1/3 bg-slate-50 dark:bg-slate-950 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 p-4 flex flex-col gap-2">
                    ...
                    {providers.map(p => (
                        <button
                            ...
                            className={`flex items-center justify-between w-full p-3 rounded-lg text-sm font-medium transition-all ${
                                activeTab === p.id 
                        ...
                        >
                            ...
                            {activeTab === p.id && <ChevronRight size={14} className="text-slate-400" />}
                        </button>
                    ))}
                ...
                <div className="flex-1 flex flex-col relative">
                    ...
                    <div className="p-8 flex-1 flex flex-col justify-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mb-8 text-sm">
                            Follow these steps to generate a new secret key for the application.
                        </p>

                        <div className="space-y-4 mb-8">
                            {currentProvider.steps.map((step, i) => (
                                <div key={i} className="flex items-center gap-4">
                                    <span className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 text-xs font-bold flex items-center justify-center flex-shrink-0">
                                        {i + 1}
                                    </span>
                        ...
                        <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-200 text-white dark:text-slate-900 rounded-lg font-bold transition-all transform hover:-translate-y-0.5 shadow-lg w-fit"
                        >
