...
const TestRunnerModal = ({ isOpen, onClose, prompt, defaultApiKey, defaultOpenAIKey, onSaveSnippet }) => {
    ...
    // <--- 3. Define the Launcher Logic
    const launchHivemind = (e) => {
        if (e) {
            e.preventDefault();
            e.stopPropagation();
        }
        console.log("🚀 Modal Launching Hivemind...", prompt);
        navigate('/hivemind', {
            state: { prompt: prompt, timestamp: Date.now() }
        });
    };
            ...
            {runner.showHelpModal && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center pointer-events-none">
                    <div className="pointer-events-auto">
                        {/* ApiKeyHelpModal would act here if needed */}
                    </div>
                </div>
            )}
        </div>
