...
import { Palette, Aperture, Layers, Wand2, FastForward, ArrowRight, Check, Circle, Zap } from 'lucide-react';

// ... (skipping renderValue function)
...
const steps = [
    ...
    { id: 'maverick', label: 'The Maverick', icon: <Zap size={14} />, value: manifest.maverick }, // Rebranded
    ...
    <div className="w-80 border-l border-slate-800 bg-slate-950 flex flex-col h-full animate-in slide-in-from-right-4 shrink-0">
                ...
                return (
                    <div key={step.id} className={`relative flex gap-3 ${isPending ? 'opacity-40' : 'opacity-100'} transition-opacity`}>
                        ...
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border z-10 
                                ${isActive ? 'bg-fuchsia-500 text-white border-fuchsia-400 shadow-[0_0_10px_rgba(217,70,239,0.5)]' :
        ...
        {showControls && (
            <div className="p-4 border-t border-slate-800 bg-slate-900/30 space-y-3">
                ...
                <button onClick={onConfirm} disabled={!isReady} className={`w-full py-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all transform active:scale-95 shadow-lg ${isReady ? 'bg-fuchsia-600 hover:bg-fuchsia-500 text-white shadow-fuchsia-900/20' : 'bg-slate-800 text-slate-600 cursor-not-allowed'}`}>
