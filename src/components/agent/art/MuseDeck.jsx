...
const MuseDeck = ({ data, onConfirm }) => {
    ...
    // Safety Check: If data is missing or array is empty, show nothing (or loading state)
    ...
    return (
        <div className="w-full mt-4 animate-in slide-in-from-bottom-2">
            ...
            <div className="bg-slate-950/50 border-x border-b border-purple-500/30 rounded-b-2xl p-4">
                ...
                {/* We use 'flex' with 'overflow-x-auto' to create the scroll strip */}
                <div className="flex gap-4 overflow-x-auto pb-4 mb-2 snap-x custom-scrollbar">
                    {data.strategy_options.map((option, idx) => (
                        <button
                            key={idx}
                            onClick={() => setSelectedOption(option.label)}
                            // CRITICAL: shrink-0 prevents squashing. w-72 sets fixed width. snap-start aligns it.
                            className={`snap-start shrink-0 w-72 group relative p-4 rounded-xl border text-left transition-all hover:scale-[1.01] flex flex-col ${selectedOption === option.label
                        ...
                        >
                            ...
                            <div className="flex items-start gap-3 mb-2">
                                ...
                                <h4 className={`text-sm font-bold ${selectedOption === option.label ? 'text-white' : 'text-slate-200'}`}>
                                    {option.label}
                                </h4>
                            </div>
