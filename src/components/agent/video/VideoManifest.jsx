...
import { Clapperboard, Video, Film, PlayCircle, FastForward, ArrowRight, Check } from 'lucide-react';

const VideoManifest = ({ manifest, currentPhase, onConfirm, onAutoPilot, isReady }) => {

    // Robust Phase Checking
    const showControls = ['strategy', 'strategy_options', 'producer', 'spec', 'spec_options', 'director'].includes(currentPhase);

    const steps = [
        { id: 'strategy', label: 'Concept', icon: <Clapperboard size={14} />, value: manifest.strategy },
        { id: 'spec', label: 'Direction', icon: <Video size={14} />, value: manifest.direction },
        { id: 'blueprint', label: 'Storyboard', icon: <Film size={14} />, value: manifest.blueprint ? 'Locked' : null },
        { id: 'final', label: 'Render', icon: <PlayCircle size={14} />, value: manifest.final ? 'Complete' : null },
    ];
    ...
    return (
        <div className="w-80 border-l border-slate-800 bg-slate-950 flex flex-col h-full animate-in slide-in-from-right-4 shrink-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {steps.map((step, idx) => {
                    // Fuzzy match for phase active state
                    ...
                    return (
                        <div key={step.id} className={`relative flex gap-3 ${isPending ? 'opacity-30' : 'opacity-100'}`}>
                            ...
                            <div className={`
                                ...
                                ${isActive ? 'bg-orange-500 text-white border-orange-400 shadow-[0_0_10px_rgba(249,115,22,0.5)]' :
                            ...
                            <div>
                                <span className={`text-xs font-bold uppercase tracking-wide block mb-0.5 ${isActive ? 'text-orange-400' : 'text-slate-500'}`}>
                                    {step.label}
                                </span>
                                {step.value ? (
