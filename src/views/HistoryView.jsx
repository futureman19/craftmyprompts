    ...
    <div className="flex-1 bg-slate-50 overflow-y-auto p-6 md:p-10 w-full pl-16 md:pl-24">
      ...
      <div className="max-w-full w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
        ...
        {sessionHistory.map((item, idx) => (
            <div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm opacity-75 hover:opacity-100 transition-opacity">
                ...
                <p className="font-mono text-xs text-slate-600 line-clamp-3">{item.prompt}</p>
                <button onClick={() => {
                    navigator.clipboard.writeText(item.prompt);
                    showToast("Copied from history");
                }} className="mt-3 text-xs text-indigo-600 font-bold hover:underline">Copy Again</button>
            </div>
        ))}
      </div>
    </div>
